class TokenManager(signing.TimestampSigner):
    ...
    def __init__(self):
        ...
        #TODO: make these configuratble parameters
        self.max_retries = 2
        self.max_age = None
        self._time_units = ['s', 'm', 'h', 'd']
        assert self.max_reties >= 1 if self.max_reties else None

        #TODO: review the whole security framework 
        key = "my secret key"  #self.settings.get('key', raise_exception=False)
        salt = None #self.settings.get('salt', raise_exception=False)
